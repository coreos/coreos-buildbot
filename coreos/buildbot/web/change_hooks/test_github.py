...
gitJsonPayloadNewBranch = """
{
  ...
  "after": "de8251ff97ee194a289832576287d6f8ad74e3d0",
  "ref": "refs/heads/copyofmaster"
}
"""

gitJsonPayloadNewTag = """
{
  "before": "0000000000000000000000000000000000000000",
  "repository": {
    "url": "http://github.com/defunkt/github",
...
"""
class TestGithubChangeHookDefaults(unittest.TestCase):
    def setUp(self):
        self.changeHook = github_resource.GithubResource(
                dialects={'coreos_github' : True})
    ...
    def testGitWithNewBranch(self):
        changeDict={"payload" : [gitJsonPayloadNewBranch]}
        self.request = FakeRequest(changeDict)
        self.request.uri = "/coreos/change_hook/github"
        self.request.method = "GET"
        d = self.request.test_render(self.changeHook)
...
class TestGithubChangeHookIncludeCopies(TestGithubChangeHookDefaults):
    ...
    def testGitWithNewBranch(self):
        ...
        def check_changes(r):
            ...
            change = self.request.addedChanges[0]
            self.assertEquals(change['files'], ['filepath.rb'])
            self.assertEquals(change["branch"], "copyofmaster")
            self.assertEquals(change["repository"], "http://github.com/defunkt/github")
